...
export class ClaudeClient {
    ...
    async fetch(settings, {cancellable = null} = {}) {
        ...
        if (isClaudeApiLogin(credentials)) {
            return createRecord('claude', 'Claude Code', {
                signedIn: true,
                tierLabel: 'API',
                accountEmail: pickAccountEmail(credentials.email),
                usageStatusText: 'Claude API login',
                authHelpText: 'This login is API / prepaid. Plan bars (5-hour and weekly) need `claude login` and the Claude.ai Pro/Max option.',
            });
        }
    ...
    async _fetchUsage(credentials, cancellable) {
        ...
        try {
            ...
            tier = formatTier(profile?.organization?.organization_type
                ...
                || tier;
            email = pickAccountEmail(profile);
        } catch (error) {
            if (error instanceof HttpError && error.authFailed)
                throw error;
        }
        return buildClaudeRecord(payload, tier, email);
    }
    ...
    async _refresh(credentials, cancellable) {
        const latest = readClaudeCredentials([credentials.path], {allowApi: false}) || credentials;
        ...
        try {
            ...
            const reread = readClaudeCredentials([credentials.path], {allowApi: false});
...
function isClaudeApiLogin(credentials) {
    if (credentials?.source === 'claude-api')
        return true;
    const scope = String(credentials?.scope || '').toLowerCase();
    if (!scope)
        return false;
    return !scope.includes('user:sessions:claude_code') && !scope.includes('user:subscription');
}
...
function buildClaudeRecord(payload, tierLabel, accountEmail = '') {
